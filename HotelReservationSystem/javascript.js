...
function appendProduct(firstName,productId) {
...
function parseMessages(responseXML) {
    ...
    if (responseXML == null) {
        ...
        if (products.childNodes.length > 0) {
            completeTable.setAttribute("bordercolor", "green");
            completeTable.setAttribute("border", "2");
    
            for (loop = 0; loop < products.childNodes.length; loop++) {
                var product = products.childNodes[loop];
                var firstName = product.getElementsByTagName("firstName")[0];
                var productId = product.getElementsByTagName("id")[0];
                appendProduct(firstName.childNodes[0].nodeValue,
